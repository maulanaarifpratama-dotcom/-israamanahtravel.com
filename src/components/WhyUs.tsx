        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {pillars.map((p, i) => (
            ...
            >
              {/* Icon circle */}
              <div
                className="w-20 h-20 mx-auto mb-5 rounded-full flex items-center justify-center text-gold transition-all duration-300 group-hover:scale-110"
                style={{
              ...
              {/* Number */}
              <p className="text-[11px] tracking-[0.4em] uppercase text-gold/60 font-semibold font-body mb-2">
                0{i + 1}
              </p>

              {/* Title */}
              <h3 className="font-heading font-bold text-dark-900 text-2xl mb-1">{p.title}</h3>
              <p className="text-gold-600 text-sm font-semibold font-body mb-3">{p.subtitle}</p>

              {/* Divider */}
              <div className="w-10 h-px bg-gold/30 mx-auto mb-4" />

              {/* Desc */}
              <p className="text-dark-700/65 text-sm leading-relaxed font-body">{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
